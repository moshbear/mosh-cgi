...
int main() {
	...
	cout << s::body_begin();
	cout << s::h1("mosh") + (s::span(s::P("class", "red"), "-cgi")) + "HTTP Cookies Test Results";
	cout << s::comment("This page generated by mosh-cgi for $ENV[HTTP_REMOTE_HOST]");
	cout << s::h4("Thanks for using mosh" + s::span(s::P("class", "red"), "-cgi")) + " $ENV[HTTP_REMOTE_HOST]($ENV[HTTP_REMOTE_ADDR])!";
	if (name != "" && value != "")
